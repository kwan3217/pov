...
#furnsh "MSL/relay.tm"
#furnsh "MSL/msl_rec.tm"

#declare fps=24;
#declare DT=1/fps;
#declare TTT=frame_number*DT+510;
//                 12345678901234567890123
//#declare RefUTC="2012-08-06 05:10:46.018UTC"; //Predicted EI according to video 00:21:57 (+-1s)
#declare RefTDB="2012-08-06 05:02:52.952612TDB"; //Start of reconstructed kernel, ~540s before EI at cruise separation
#declare RefUTC="2012-08-06 05:01:45.769478UTC"; //Same time, in UTC
#declare EITDB="2012-08-06 05:11:52.840108TDB"; //Reconstructed first sample below EI, 3522.2km from center of Mars
#declare LandTDB="2012 AUG 06 05:19:04.475TDB"; //Official landing time according to msl_edl_v01.bsp comment
#declare RefET=str2et(RefTDB);
#declare EIET=str2et(EITDB);
#declare LandET=str2et(LandTDB);
#declare StartET=EIET-30;
PrintNumber("StartET: ",StartET)
PrintNumber("RefET:   ",RefET)
PrintNumber("LandET:  ",LandET)
#declare h0=val(substr(RefUTC,12,2));
#declare m0=val(substr(RefUTC,15,2));
#declare s0=val(substr(RefUTC,18,6));
#declare RefUTCSOD=h0*3600+m0*60+s0;
#declare h=floor((TTT+RefUTCSOD)/3600);
#declare m=floor(((TTT+RefUTCSOD)-h*3600)/60);
#declare s=(TTT+RefUTCSOD)-h*3600-m*60;
//These times are from the Spice kernel, therefore relative to RefET
#declare Tebm0=780;       //Time of first entry balance mass jettison
#declare dt_ebm=2;        //Time between entry balance mass jettisons
#declare Tdeploy=799.166; //Time of parachute deploy shock
//#declare Tinflate=Tdeploy+; //Time of start of parachute inflation
#declare Theatshield=818.90; //Time of heatshield jettison
#declare Tsep=915.796;  //Time of backshell jettison
#declare Tland=LandET-RefET;       //Time of touchdown
PrintNumber("TLand: ",Tland)
#declare Tflyaway=Tland+1;      //Time of descent stage flyaway start
#declare Tlastatt=540+431+9/24;     //Time of last attitude in CK kernel - after this, use a fixed attitude
#declare TEnd=final_frame*DT+(StartET-RefET);
PrintNumber("TEnd:  ",TEnd)
...
#macro R(TT)  //Position vector in Mars centered, Mars body fixed, in meters
  (1000*spkezr("MSL",RefET+TT,"IAU_MARS","NONE","499")) 
#end
#macro RI(TT)  //Position vector in Mars centered inertial, in meters
  (1000*spkezr("MSL",RefET+TT,"MARSIAU","NONE","499")) 
#end
#macro V(TT)  //Velocity vector in Mars centered, Mars body fixed, in m/s
  #local vel=<0,0,0>;
  #local junk=spkezr("MSL",RefET+TT,"IAU_MARS","NONE","499",vel);
  (1000*vel) 
#end
#macro VI(TT)  //Velocity vector in Mars centered inertial, in m/s
  #local vel=<0,0,0>;
  #local junk=spkezr("MSL",RefET+TT,"MARSIAU","NONE","499",vel);
  (1000*vel) 
#end
...
#macro A(TT)       //Non-gravitational acceleration in rotating frame
  #local QQ=pxform("MARSIAU","IAU_MARS",RefET+TT);
  (QuatVectRotate(QQ,Ang(TT)))
#end

#macro S(TT) (1000*spkezr("SUN",RefET+TT,"IAU_MARS", "LT+S","MSL")) #end
#macro E(TT) (1000*spkezr("399",RefET+TT,"IAU_MARS","XLT+S","MSL")) #end
#macro MEX(TT) (1000*spkezr("-41",RefET+TT,"IAU_MARS","XLT+S","MSL")) #end //Mars Express
#macro MRO(TT) (1000*spkezr("-74",RefET+TT,"IAU_MARS","XLT+S","MSL")) #end //Mars Recon Orbiter
#macro ODY(TT) (1000*spkezr("-53",RefET+TT,"IAU_MARS","XLT+S","MSL")) #end //Mars Odyssey
#macro LT(TT) (vlength(E(TT))/299792458) #end
#declare hlt=floor((TTT+RefUTCSOD+LT(TTT))/3600);
#declare mlt=floor(((TTT+RefUTCSOD+LT(TTT))-hlt*3600)/60);
#declare slt=(TTT+RefUTCSOD+LT(TTT))-hlt*3600-mlt*60;
#macro Q(TT) (pxform("MSL_ROVER","IAU_MARS",RefET+min(TT,Tlastatt))) #end
...
#declare GaleMesh=mesh2 {
  ...
  CenterWorld(TTT)
  translate (7-3.554)*vnormalize(R(Tland))
}
...
cylinder {
  <0,0,0>,V(TTT)/fps,0.1
  pigment {color rgbt <1,1,1,BLinterp(Tebm0-7,0,Tebm0-2,1,TTT)>}
  no_shadow
}
#end
...
#if(TTT<Tflyaway)
object {
  MSLDescentStage(Throttle*(NMLE>4),Throttle) translate z*RappelDist
  scale <1,1,-1>
...
#end
object {
  MSLRover(RoverCf,RoverCa,RoverAf)
  scale <1,1,-1>
...
#declare ADist=ASDR(Tdeploy-4,Tdeploy-2,Tdeploy+10,Tdeploy+15,0,30,TTT);
#declare NoDist=ASDR(Tdeploy-4,Tdeploy-2,Tdeploy+10,Tdeploy+15,10,40,TTT);
...
PrintNumber("Csound: ",csound(TTT))

union {
  textHud(concat("06 Aug 2012 ",str(h,-2,0),":",str(m,-2,0),":",str(s,-6,3),"UTC SCET (",str(TTT-(EIET-RefET),7,3),"s)"),
          <-16,-8.5>,<1,1,1>,0,-1)           
  ...
  textHudNumber(agl(TTT)-RappelOfs, "m",<-12,8.5>,BLinterp(Tland+2,<1,0,0,0>,Tland+4,<1,0,0,1>,TTT),  -1)
  ...
  textHud("Scorch:",                     <-16,7.0>,<1,0.5,0>,0,-1)
  textHudNumber(DynScorch, "J/cm^2",<-12,7.0>,<1,0.5,0>,  -1)
